	...
	partnerSelected.partnership.forEach((partner) => {
		...
		if (partner.agreements) {
			...
			partner.agreements.forEach((agreement) => {
				const listAgree = document.createElement("h5");
				listAgree.classList.add(
					"card-subtitle",
					"mb-1",
					"mt-2",
					"agreement",
				...
				listAgree.innerHTML = agreement.typeAgreement
					? `${agreement.typeAgreement}`
					: "";
				const time = document.createElement("p");

				time.classList.add("card-text", "time", "mb-1");
				time.innerHTML = `Signed in ${agreement.year}`;

				partnershipCard.appendChild(listAgree);
				partnershipCard.appendChild(time);
				const source = document.createElement("a");
				source.classList.add("card-link",);
				source.href = agreement.linkAgreement
					? `${agreement.linkAgreement}`
					: `${agreement.sources}`;
				source.target = "_blank";
				source.innerHTML = "Source";
				partnershipCard.appendChild(source);
			});
			...
			partnerAgreement.classList.add(
				"card-subtitle",
				"mb-1",
				"agreement",
			);
...
export function populateMultilateral(multiData, selectedBloc, multiJsonData) {
	console.log("Multi JSON data to populate", selectedBloc);
	...
	blocSource.href = multiJsonData.find(
		(item) => item.blocName === prueba
	).link;
